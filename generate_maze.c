...
int is_visitable(int x, int y, int maze[][x], int curr_x, int curr_y, struct Cell_coords last_cell){
...
void generate_maze(int start_x, int start_y, int end_x, int end_y, int x, int y, int maze[][x]){
    ...
    struct Cell_coords curr_copies[x*y];
    ...
    do{
        ...
        is_visitable(x, y, maze, curr.x+1, curr.y, curr)){
            struct Cell_coords possible_cell;
            ...
            curr.x = (*(struct Cell_coords*)stack_at(winning_path, 0)).x;
            curr.y = (*(struct Cell_coords*)stack_at(winning_path, 0)).y;
            stack_pop(winning_path);
        }

        if(stack_size(winning_path) == 1 && // If the path does not rech the end, it starts over
        !is_visitable(x, y, maze, curr.x, curr.y-1, curr) &&
        !is_visitable(x, y, maze, curr.x-1, curr.y, curr) &&
        !is_visitable(x, y, maze, curr.x, curr.y+1, curr) &&
        !is_visitable(x, y, maze, curr.x+1, curr.y, curr)){
    ...
    struct Cell_coords past_cells_copies[x*y];
    ...
    for(int i =  stack_size(winning_path)-1 ; i >= 0 ; i--){ //creates all the paths that does not reach the end
        ...
        curr.x = (*(struct Cell_coords*)stack_at(winning_path, i)).x;
        curr.y = (*(struct Cell_coords*)stack_at(winning_path, i)).y;
        ...
        stack_size(past_cells) > 1){
            ...
            if((is_visitable(x, y, maze, curr.x, curr.y-1, curr)) ||
                ...
                curr.x = (*(struct Cell_coords*)stack_at(past_cells, 0)).x;
                curr.y = (*(struct Cell_coords*)stack_at(past_cells, 0)).y;
